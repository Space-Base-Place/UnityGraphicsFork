#ifndef UNIVERSAL_PARTICLES_FORWARD_SLIT_PASS_INCLUDED
#define UNIVERSAL_PARTICLES_FORWARD_SLIT_PASS_INCLUDED
...
inline void InitializeSurfaceData(ParticleParams particleParams, out SurfaceData surfaceData)
{
...
VaryingsParticle vertParticleShadowLit(AttributesParticle input)
{
...
half4 fragParticleShadowLit(VaryingsParticle input) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(input);
    ...
    half4 finalColor = half4(albedo * mainLight.shadowAttenuation + surfaceData.emission, surfaceData.alpha);

    #ifdef _USE_ATMOSPHERIC_GLOBAL_ILLUMINATION
    finalColor += half4(SampleAtmosphericIllumination(inputData.positionWS, inputData.normalWS).rgb * albedo, 1);
    #endif
...
#endif // UNIVERSAL_PARTICLES_FORWARD_SLIT_PASS_INCLUDED
